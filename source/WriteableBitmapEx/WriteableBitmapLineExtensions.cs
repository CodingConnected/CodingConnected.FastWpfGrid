 ...
 static partial class WriteableBitmapExtensions
    {
        ...
        public static void DrawLineDDA(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int color)
        {
            using (var context = bmp.GetBitmapContext())
            {
                ...
                if (len != 0)
                {
                    ...
                    var incx = dx / (float)len;
                    var incy = dy / (float)len;
                    float x = x1;
                    float y = y1;
        ...
        public static void DrawLine(BitmapContext context, int pixelWidth, int pixelHeight, int x1, int y1, int x2, int y2, int color)
        {
            ...
            { // x increases by +/- 1
                ...
                // Init steps and start
                var incy = (dy << PRECISION_SHIFT) / dx;

                var y1s = y1 << PRECISION_SHIFT;
                var y2s = y2 << PRECISION_SHIFT;
                var hs = pixelHeight << PRECISION_SHIFT;
                ...
                // Walk the line!
                var y = ys >> PRECISION_SHIFT;
                var previousY = y;
                var index = x1 + y * pixelWidth;
                var k = incy < 0 ? 1 - pixelWidth : 1 + pixelWidth;
                for (var x = x1; x <= x2; ++x)
                {
                    pixels[index] = color;
                    ys += incy;
            ...
            else
            {
                ...
                // Init steps and start
                var x1s = x1 << PRECISION_SHIFT;
                var x2s = x2 << PRECISION_SHIFT;
                var ws = pixelWidth << PRECISION_SHIFT;

                var incx = (dx << PRECISION_SHIFT) / dy;
                ...
                var index = x1s;
                var indexBaseValue = y1 * pixelWidth;
        ...
        public static void DrawLinePenned(BitmapContext context, int w, int h, int x1, int y1, int x2, int y2, BitmapContext pen)
        {
            ...
            var size = pen.WriteableBitmap.PixelWidth;
            var pw = size;
        ...
        public static void DrawLineWu(BitmapContext context, int pixelWidth, int pixelHeight, int x1, int y1, int x2, int y2, int sa, int sr, int sg, int sb)
        {
            ...
            if (DeltaY > DeltaX)
            {
                ...
                while (--DeltaY != 0)
                {
                    ...
                    var weight = Weighting ^ WEIGHT_COMPLEMENT_MASK;
        ...
        internal static bool CohenSutherlandLineClip(Rect extents, ref double x0, ref double y0, ref double x1, ref double y1)
        {
            ...
            var outcode0 = ComputeOutCode(extents, x0, y0);
            var outcode1 = ComputeOutCode(extents, x1, y1);
