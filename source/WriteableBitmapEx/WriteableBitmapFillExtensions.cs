 ...
 static partial class WriteableBitmapExtensions
    {
        ...
        public static void FillRectangle(this WriteableBitmap bmp, int x1, int y1, int x2, int y2, int color, bool doAlphaBlend = false)
        {
            using (var context = bmp.GetBitmapContext())
            {
                ...
                var noBlending = !doAlphaBlend || sa == 255;
        ...
        public static void FillPolygonsEvenOdd(this WriteableBitmap bmp, int[][] polygons, int color)
        {
            ...
            using (var context = bmp.GetBitmapContext())
            {
                ...
                foreach (var points in polygons)
                {
                    ...
                    for (var i = start; i < pn; i += 2)
                    {
                        var px = points[i];
                        var py = points[i + 1];
                        if (py != lastY)
                        {
                            var edge = new Edge(lastX, lastY, px, py);
                ...
                edges.Sort();
                var yMin = edges[0].StartY;
                if (yMin < 0) yMin = 0;

                var intersectionsX = new int[edges.Count];

                var currentEdges = new LinkedList<Edge>();
                var e = 0;
